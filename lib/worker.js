...
(function() {
  var Promise, _, clone, follow, logger, sortErrorResults, utils, x;
  ...
  x.get_handlers = function(handlers, entry, doc_type) {
     ...
     */
    var handler, ref;
    handler = (ref = handlers[doc_type]) != null ? ref[entry.a] : void 0;
    if (handler) {
      return {
        "null": handler
      };
  ...
  x.get_plugin_handlers = function(handlers, entry, doc_type) {
     ...
     */
    var entry_plugin, filtered_handlers, handler, plugin, plugin_handlers, ref, ref1, ref2, ref3, ref4;
    filtered_handlers = {};
    for (plugin in handlers) {
      plugin_handlers = handlers[plugin];
      handler = (ref = plugin_handlers[doc_type]) != null ? ref[entry.a] : void 0;
      if (!handler) {
        entry_plugin = entry.plugin || entry.resource;
        if (entry_plugin === plugin) {
          handler = (ref1 = plugin_handlers[doc_type]) != null ? (ref2 = ref1.self) != null ? ref2[entry.a] : void 0 : void 0;
        } else {
          handler = (ref3 = plugin_handlers[doc_type]) != null ? (ref4 = ref3.other) != null ? ref4[entry.a] : void 0 : void 0;
        }
      }
      if (handler) {
  ...
  x.get_audit_entries_to_sync = function(doc) {
    ...
    return _.filter(doc.audit, function(entry) {
      var ref;
      return !entry.synced && ((!((ref = entry.attempts) != null ? ref.length : void 0)) || entry.attempts[0] < now);
    });
  };
  ...
  x.update_document_with_worker_result = function(doc) {
    return function(result) {
      var data_path, existing_data, new_data, ref, ref1, ref2, ref3;
      data_path = ((ref = result.value) != null ? ref.path : void 0) || ((ref1 = result.error) != null ? ref1.path : void 0);
      new_data = ((ref2 = result.value) != null ? ref2.data : void 0) || ((ref3 = result.error) != null ? ref3.data : void 0);
      ...
      return Promise.hashAll(entry_promises).then(function(results) {
        return x.update_audit_entries(db, doc._id, results).then(function() {
          ...
          _.each(results, function(actionResults, actionId) {
            var actionLog, ref, rejected, resolved;
            ref = sortErrorResults(actionResults), resolved = ref[0], rejected = ref[1];
            actionLog = docLog.child({
              actionId: actionId
            });
