    ...
    $SISYPHUS
      ...
      |- src: coffeescript source files go here
          ...
          |- loggers.coffee: web and worker [bunyan](https://github.com/trentm/node-bunyan) loggers,
             ...
             You will need to modify your config to send the logs to the appropriate location (usually a file when in production).
...
A couple things:
  ...
  * **logging:** 
    ...
    you can use the logger, which is passed as the third argument to your worker handler.
    ...
    The relevant metadata linking your log entry to the particular document/revision action with which it is being called has already been included in the logger, so you do not have to add this metadata.
