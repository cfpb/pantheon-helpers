...
(function() {
  var Promise, _, exec, parse_links, request, resolve, timers, utils,
    slice = [].slice;
  ...
  Promise.setTimeout = Promise.denodeify(function() {
    var args, callback, delay, i;
    delay = arguments[0], args = 3 <= arguments.length ? slice.call(arguments, 1, i = arguments.length - 1) : (i = 1, []), callback = arguments[i++];
    return timers.setTimeout.apply(null, [callback, delay].concat(args));
  });

  Promise.exec = Promise.denodeify(exec);

  Promise.sendHttp = function(promise, resp) {
    return promise.then(function(doc) {
      return resp.send(JSON.stringify(doc));
    })["catch"](function(err) {
      if (err.statusCode) {
        return resp.status(err.statusCode).send(JSON.stringify(_.pick(err, 'error', 'reason', 'description', 'msg')));
      } else {
